...
namespace MackerelPluginSet.BanTables {
	public static class BanPredicates {
		...
		static bool? JudgeName_Core(string s, string arg) {
			try {
				Regex r = new Regex(arg);
				return r.IsMatch(s);
			}
			catch (ArgumentException) {
				TShockAPI.TShock.Log.Error("Regex parameter error. Invalid syntax : \"{0}\"", arg);
				return null;
			}
		}
		...
		static bool? JudgeIP_Core(string s, string arg) {
			...
			try {
				...
				TShockAPI.TShock.Log.Error("IP parameter error. Invalid format : \"{0}\" or \"{1}\"", s, arg);
				return null;
			}
		}

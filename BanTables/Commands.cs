		...
		static void OnCommand(TS.CommandArgs args) {
			...
			if (mode == "list") {
				...
				try {
					...
					TS.TShock.Log.Error(ex.ToString());
				...
				foreach (var b in banList) {
					if (b.PredicateName == pred) {
						...
						string msg = string.Format("#{0} : test by \"{1}\" => {2}", b.Priority, b.Parameter, matched.HasValue ? matched.Value ? b.AllowJudge ? "Allow" : "Deny" : "Unmatched" : "Error");
